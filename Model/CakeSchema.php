	...
	public function read($options = array()) {
		...
		if (is_array($models)) {
			foreach ($models as $model) {
				...
				// Evoluted checks for overridden plugin model.
				// Prevents the model appearing in app migration when it's handled by the plugin
				if (is_object($Object)) {
					$pluginName = str_replace($Object->name, '', $importModel);
 ...
 */
	public function write($object, $options = array()) {
		...
		$out = "App::uses('CakeSchema', 'Migrations.Model');\n\n";
